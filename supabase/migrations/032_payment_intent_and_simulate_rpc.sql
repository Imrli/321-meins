-- Fehlende Spalten auf Remote-DB (Migration 019 evtl. nicht angewendet) + Zahlungs-RPCs.
...
COMMENT ON COLUMN public.auktionen.payment_intent_id IS
  'Stripe PaymentIntent (optional im Testmodus).';
COMMENT ON COLUMN public.transporteure.stripe_connect_account_id IS
  'Stripe Connect Account ID für Auszahlung.';

-- simulate_auktion_zahlung: setzt payment_intent_id nur wenn Spalte existiert (immer nach ALTER oben).
