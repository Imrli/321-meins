...
export function UmzugForm({
  onCancel,
  addAuction,
  goToMyAuctions,
  notesContactBlocked = () => false,
}: UmzugFormProps) {
  const [form, setForm] = useState<UmzugFormState>(initialUmzugForm);
  const [errors, setErrors] = useState<Set<keyof UmzugFormState>>(new Set());
  ...
  const notizenBlocked = useMemo(
    () => notesContactBlocked(form.notizen),
    [form.notizen, notesContactBlocked],
  );

  const update = (name: string, value: string | boolean) => {
    setForm((prev) => ({ ...prev, [name]: value }));
    setSubmitError(null);
    if (typeof value === "string" && errors.has(name as keyof UmzugFormState)) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const missing = collectUmzugMissing(form);
    setErrors(missing);
    if (missing.size > 0) {
      setShowError(true);
      return;
    ...
    try {
      const newId = await addAuction({
        startort: form.von.trim(),
        zielort: form.nach.trim(),
        empfVorname: "–",
        empfName: "–",
        empfStrasse: form.nach.trim(),
        empfPlz: "–",
        empfOrt: "–",
        abholdatum,
        lieferdatum: abholdatum,
        lieferzeitpraeferenz: zeitfensterToLieferzeit(
          form.zeitfenster as UmzugZeitfenster,
        ),
        notizen: form.notizen.trim() || undefined,
        durationMs: durationResult.durationMs,
        umzugDetails: buildUmzugDetails(form),
      });
      if (!newId) {
        setSubmitError(
  ...
  if (success) {
    return (
      <div id="umzug-erfassen" className="mt-8 rounded-3xl border border-emerald-200 bg-emerald-50 p-8 anim-fade-up">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-emerald-800">
              Dein Umzug von <strong>{form.von}</strong> nach <strong>{form.nach}</strong> ist live.
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" onClick={() => { setForm(initialUmzugForm); setDuration(DEFAULT_AUCTION_DURATION); setSuccess(false); goToMyAuctions(); }} className="inline-flex items-center gap-2 rounded-full bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 transition-colors btn-action-shine">
                Zu meinen Auktionen <ArrowIcon className="size-4" />
              </button>
              <button type="button" onClick={() => { setForm(initialUmzugForm); setDuration(DEFAULT_AUCTION_DURATION); setSuccess(false); onCancel(); }} className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-emerald-700 ring-1 ring-emerald-200 hover:bg-emerald-100 transition-colors">
  ...
  return (
    <form id="umzug-erfassen" onSubmit={onSubmit} noValidate className="mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm anim-fade-up">
      <div className="border-b border-slate-100 px-7 py-5 md:px-10">
        ...
        <p className="mt-1 text-sm text-[var(--color-muted)]">
          <span className="inline-flex items-center rounded-full bg-[var(--color-brand-50)] px-2.5 py-0.5 text-xs font-semibold text-[var(--color-brand-700)] ring-1 ring-[var(--color-brand-100)]">Umzug</span>
          <span className="ml-2">– Anbieter unterbieten sich live in der Auktion.</span>
        </p>
      </div>
      <div className="space-y-8 px-7 py-7 md:px-10 md:py-9">
        <UmzugFormFields form={form} errors={errors} onChange={update} notizenBlocked={notizenBlocked} />
        ...
        <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:items-center sm:justify-end">
          ...
          <button type="submit" disabled={notizenBlocked} className="inline-flex items-center justify-center gap-2 rounded-full bg-[var(--color-accent-500)] px-7 py-3.5 text-sm font-semibold text-white shadow-lg shadow-orange-900/30 transition-colors hover:bg-[var(--color-accent-600)] disabled:cursor-not-allowed disabled:opacity-50 btn-action-shine">
